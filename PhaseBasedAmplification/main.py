...
def main(colorsp):
    ...
    print(f"\nProcessing {video_name} " f"and saving results to {videoSavePath} \n")
    ...
    if colorspace == "luma1":
        ...
        def colorspaceFunction(x):
            return bgr2yiq(x)[:, :, 0]

        def invColorspaceFunction(x):
            return cv.cvtColor(
        ...
        def invColorspaceFunction(x):
            return cv.cvtColor(
                cv.normalize(yiq2rgb(x), None, 0, 255, cv.NORM_MINMAX, cv.CV_8UC3),
                cv.COLOR_RGB2BGR,
            )
    ...
    frames, videoSampleFreq = get_video(videoPath, scaleFactor, colorspaceFunction)
    ...
    if sampleFrequency > 0.0:
        ...
        print(f"Sample Frequency overriden with input!: frameRate = {frameRate} \n")
    else:
        frameRate = videoSampleFreq
        print(f"Detected Sample Frequency: frameRate = {frameRate} \n")

    # Get Bandpass Filter Transfer function
    transferFunction = bandpass_filter(
        freqLowerbound, freqHigherBound, frameRate, nFrames, DEVICE
    )

    # Get Complex Steerable Pyramid Object
    max_depth = int(np.floor(np.log2(np.min(np.array((refWidth, refHeight))))) - 2)
    ...
    filtersTensor = torch.tensor(np.array(filters)).type(torch.float32).to(DEVICE)
    ...
    if (filtersTensor.shape[0] % batchSize) != 0:
        print(
            f"WARNING! Selected Batch size: {batchSize} might "
            f"not be compatible with the number of "
            f"Filters: {filtersTensor.shape[0]}! \n"
        )

    # Compute DFT for all Video Frames
    framesTensor = torch.tensor(np.array(frames)).type(torch.float32).to(DEVICE)
    ...
    if colorspace == "yiq" or colorspace == "rgb":
        ...
        for c in range(framesTensor.shape[-1]):
            ...
            resultVideo[:, :, :, c] = phaseAmplified.process_single_channel(
                framesTensor[:, :, :, c], filtersTensor, video_dft
            )
        ...
        resultVideo[:, :, :, 0] = phaseAmplified.process_single_channel(
            framesTensor[:, :, :, 0], filtersTensor, video_dft
        )

    else:
        # process single channel
        video_dft = get_fft2_batch(framesTensor).to(DEVICE)
        resultVideo = phaseAmplified.process_single_channel(
            framesTensor, filtersTensor, video_dft
        )
    ...
    for vid_idx in range(nFrames):
        ...
        # stack frames
        stacked = np.hstack((bgrFrame, middle, bgrProcessed))
